...
/* randomgraph.c
   Usage: randomgraph n m
          randomgraph n m seed

   generate a random graph of n vertices and m edges (unidrected) C.C. McGeoch 2011
   Output format (directed):
...
int main(int argc, char *argv[]) 
{
  ...
  { /* for each source */
    ...
    {                           /* for each (s,d) with d < s */
      prob = (k - s) / (r - c); /* probability of selecting this one */
      double p = drand48();

      if (p < prob)
      { /* random selection with probability prob*/
        nodes[src][dst] = 1; 
        nodes[dst][src] = 1;
        //  printf("select %d %d\n", src, dst);
        s++;
